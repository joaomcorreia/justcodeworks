...
import { useUtilitiesPageConfig } from "@/store/utilities-page-config";
import {
...
const SECTION_OPTIONS: SectionOption[] = defaultUtilitiesPageConfig.sections.map(
  (section) => ({
    id: section.id,
...
export function UtilitiesEditorPanel() {
  const { editMode } = useEditMode();
  const { config, updateField, resetConfig } = useUtilitiesPageConfig();
  const [selectedSectionId, setSelectedSectionId] =
    useState<UtilitiesSectionId>("jcw-main-utilities-hero01");
    ...
    <aside className="fixed right-3 top-20 z-40 w-72 rounded-2xl border border-slate-800 bg-slate-950/95 p-3 text-xs text-slate-100 shadow-xl backdrop-blur">
      ...
      <div className="mb-3">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedSectionId(e.target.value as UtilitiesSectionId)
          }
