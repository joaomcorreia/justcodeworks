...
import { useWebsitesPageConfig } from "@/hooks/use-websites-page-config";
import { defaultWebsitesPageConfig } from "@/config/websites-page-config";
import { WebsitesSectionId } from "@/config/websites-page-sections";
...
const SECTION_OPTIONS: SectionOption[] = defaultWebsitesPageConfig.sections.map(
  (section) => ({
    id: section.id,
    label: section.internalName || section.title || section.id,
  }),
);

export function WebsitesEditorPanel() {
  const { editMode } = useEditMode();
  const { config, saveField } = useWebsitesPageConfig();
  const [selectedSectionId, setSelectedSectionId] =
    useState<string>("jcw-main-websites-hero01");
    ...
    <aside className="fixed right-3 top-20 z-40 w-72 rounded-2xl border border-slate-800 bg-slate-950/95 p-3 text-xs text-slate-100 shadow-xl backdrop-blur">
      ...
      <div className="mb-3">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedSectionId(e.target.value as WebsitesSectionId)
          }
          ...
          {selectedSection.fields.map((field) => (
            <div key={field.id}>
              ...
              <textarea
                ...
                value={field.value}
                onChange={(e) =>
                  saveField(selectedSection.id, field.id, e.target.value)
                }
                className="w-full rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-[11px] text-slate-100 outline-none focus:border-yellow-400"
              />
            </div>
