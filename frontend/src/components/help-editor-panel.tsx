...
import { useHelpPageConfig } from "@/store/help-page-config";
import {
...
const SECTION_OPTIONS: SectionOption[] = defaultHelpPageConfig.sections.map(
  (section) => ({
    id: section.id,
...
export function HelpEditorPanel() {
  const { editMode } = useEditMode();
  const { config, updateField, resetConfig } = useHelpPageConfig();
  const [selectedSectionId, setSelectedSectionId] =
    useState<HelpSectionId>("jcw-main-help-hero01");
    ...
    <aside className="fixed right-3 top-20 z-40 w-72 rounded-2xl border border-slate-800 bg-slate-950/95 p-3 text-xs text-slate-100 shadow-xl backdrop-blur">
      ...
      <div className="mb-3">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedSectionId(e.target.value as HelpSectionId)
          }
