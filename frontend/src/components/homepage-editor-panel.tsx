...
const SECTION_OPTIONS: SectionOption[] = defaultHomepageConfig.sections.map(
  (section) => ({
    id: section.id,
...
export function HomepageEditorPanel() {
  ...
  const { config, ready, updateField, toggleHeroParticles, resetConfig } =
    useHomepageConfig();
  const [selectedSectionId, setSelectedSectionId] =
    useState<HomepageSectionId>("jcw-main-hero01");
    ...
    <aside className="fixed right-3 top-20 z-40 w-72 rounded-2xl border border-slate-800 bg-slate-950/95 p-3 text-xs text-slate-100 shadow-xl backdrop-blur">
      ...
      <div className="mb-3">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedSectionId(e.target.value as HomepageSectionId)
          }
      ...
      <div className="mt-3 border-t border-slate-800 pt-2">
        <div className="mb-1 flex items-center justify-between gap-2">
          ...
          <button
            type="button"
            onClick={() =>
              toggleHeroParticles(!config.heroSettings?.enabled)
            }
            className={[
              "rounded-full px-2 py-0.5 text-[10px] font-medium",
              config.heroSettings?.enabled
                ? "bg-emerald-400 text-slate-900"
                : "border border-slate-700 bg-slate-900 text-slate-300",
            ].join(" ")}
          >
            {config.heroSettings?.enabled ? "On" : "Off"}
          </button>
        </div>
        <p className="text-[10px] text-slate-500">
          Later this will be controlled from the admin. For now it just stores the choice
          in your browser.
        </p>
      </div>
    </aside>
