// [TEMPLATE-LAB] Garage Hero Section Component (Placeholder)
//
// A hero section specifically designed for automotive garage websites.
// This is a placeholder to demonstrate the Template Lab extensibility.
//
// Expected Fields from Backend:
//   - title: Garage name/main headline
...
export default function GarageHero({ section, mode = 'public' }: SectionProps) {
  const { fields } = section;
  
  // Extract field values
  const title = getFieldValue(fields, 'title') || getFieldValue(fields, 'headline') || 'Your Trusted Garage';
  const tagline = getFieldValue(fields, 'tagline') || getFieldValue(fields, 'subtitle') || 'Expert automotive services you can rely on';
  const backgroundImage = getFieldValue(fields, 'background_image') || getFieldValue(fields, 'image');
  const ctaText = getFieldValue(fields, 'cta_text') || 'Get Quote';
  const ctaLink = getFieldValue(fields, 'cta_link') || '#quote';
  ...
  return (
    <section className="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden">
      ...
      {!backgroundImage && (
        <div className="absolute inset-0 bg-gradient-to-br from-gray-800 via-blue-800 to-gray-900"></div>
      )}
      
      {/* Development Mode Indicator */}
      {process.env.NODE_ENV === 'development' && (
        <div className="absolute top-4 left-4 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded">
          GarageHero ({section.identifier}) [PLACEHOLDER]
        </div>
      )}
    </section>
  );
